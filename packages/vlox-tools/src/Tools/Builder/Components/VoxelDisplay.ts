            ...
            return IntProp(node.id, {
              initialize(schemaNode) {
                schemaNode.enableProxy(
                  () => voxelSchema.state.getNumber(node.id),
                  (value) => {
        ...
        ...voxelSchema.mod.nodes.map((node) => {
          if (!node.valuePalette) {
            return IntProp(node.id, {
              initialize(schemaNode) {
                schemaNode.enableProxy(
                  () => voxelSchema.mod.getNumber(node.id),
                  (value) => {
  ...
  const loadIn = () => {
    ...
    if (shapeStateSchema) {
      voxelSchema.state.nodes.forEach((node) => {
        ...
        shapeStateSchema[node.id] = voxelSchema.state.getNumber(node.id);
      });
    }
    if (voxelModSchema) {
      voxelSchema.mod.nodes.forEach((node) => {
        ...
        voxelModSchema[node.id] = voxelSchema.mod.getNumber(node.id);
      });
    }
  };
